...
namespace Webscraper
{
    class Scraper
    {
        static void Main(string[] args)
        {
            ...
            //scrape option methods
            void generateYTVideos()
            {
                //give search term
                ...
                foreach (var video in videos)
                {
                    ...
                    switch (views)
                    {
                        case string a when a.Contains("K"): viewsAmount = Regex.Replace(a, @"K.*", "000"); viewsAmount = viewsAmount.Replace(",", ""); break;
                        case string b when b.Contains("mln."): viewsAmount = Regex.Replace(b, @" mln.*", "000000"); viewsAmount = viewsAmount.Replace(",", ""); break;
                        default: viewsAmount = Regex.Replace(views, @"\s.*", ""); break;
                    }
